...
void sort_timer_list::tick() { //依次遍历所有的节点，当expire小于当前的时间就调用回调函数
    if (!head)
    {
        return;
    }
...
{
    //为保证函数的可重入性，保留原来的errno,防止线程切换后把全局变量error改变了产生
    int save_errno = errno;
    int msg = sig;
    send(u_pipefd[1], (char *)&msg, 1, 0); //信号处理函数就是将传入的信号传给管道的另外一边
    errno = save_errno;
}
...
void Utils::add_sig(int sig, void(*handler)(int), bool restart) {
    ...
    if (restart){
     sa.sa_flags |= SA_RESTART; //中断有关。当有信号打断了某一个系统调用 使其返回-1 ，我们设置这个后可以让系统调用重新调用。
    }
